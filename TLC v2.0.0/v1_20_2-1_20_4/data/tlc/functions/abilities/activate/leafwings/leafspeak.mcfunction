...
execute if entity @s at @s run tellraw @a[tag=eoflib.debug, scores={eoflib.debug_mode=3..}, distance=0.1..] [{"text": "[", "color": "dark_gray"}, {"text": "Debug", "color": "gold"}, {"text": "] - ", "color": "dark_gray"}, {"text": "tlc:leafwings/leafspeak.mcf", "color": "gold", "hoverEvent": {"action": "show_text", "value": [{"selector": "@s", "color": "aqua"}, {"text": " ran the following function:\n- tlc:abilities/activate/leafwings/leafspeak.mcfunction", "color": "aqua"}]}}]
...
execute summon marker summon marker summon marker run help me
tag @e[type=marker, limit=3, sort=nearest, distance=..0.1] add tlc.leafspeak.poison_point
spreadplayers ~ ~ 12 7 false @e[sort=nearest, limit=3, type=marker, tag=tlc.leafspeak.poison_point]

            ### Summon Poison Points
execute at @e[sort=nearest, limit=3, type=marker, tag=tlc.leafspeak.poison_point] run summon area_effect_cloud ~ ~ ~ {Particle: {type: 'dust', color: [0.37, 0.49, 0.09], scale: 1}, Radius: 3f, RadiusPerTick: 0, Duration: 160, potion_contents:{custom_effects: [{id: "minecraft:poison", duration: 40, amplifier: 3, show_particles: 0b}, {id: "minecraft:slowness", duration: 40, amplifier: 1, show_particles: 0b}]}}
...
scoreboard players operation @s[tag=!eoflib.cooldown.bypass] tlc.abilities.leafspeak = #cooldown tlc.abilities.leafspeak
